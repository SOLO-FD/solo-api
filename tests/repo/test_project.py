...
class TestProjectModelCase:
    ...
    async def test_project_model_create_by_repo(self, project_repo):
        ...
        proj_from_repo = await project_repo.create(proj)
    ...
    async def test_project_model_update_by_repo(self, project_repo, default_project):
        ...
        # Act: Update by repo
        updated_proj = await project_repo.update(default_project)
    ...
    async def test_project_model_add_new_attachments_by_repo(
        self, session, project_repo, default_project
    ):
    ...
    async def test_project_model_remove_attachments_by_repo(
        self, project_repo, default_project
    ):
        # Arrange: Add new attachments
        NEW_ATTCHMENTS = 5
        new_attchments = []
        ...
        final_proj = await project_repo.update(updated_proj)
    ...
    async def test_project_model_get_by_repo(self, project_repo, default_project):
        # Act: Get by repo
    ...
    async def test_project_model_list_by_owner_id_by_repo(
        self, project_repo, default_project
    ):
        ...
        for _ in range(NEW_PROJECTS):
            new_projects.append(await create_project(project_repo, owner_id=owner_id))

        # Act: Get projects owned by the same owner
        projects_from_repo = await project_repo.list_by_owner_id(owner_id)
    ...
    async def test_project_model_remove_by_repo(self, project_repo, default_project):
        ...
        await project_repo.delete_by_id(default_project.id)

        # Assert: Delete proj should not be got, raise ValueError
        with pytest.raises(ValueError):
            await project_repo.get_by_id(default_project.id)

    # === SAD tests following ===

    async def test_project_model_create_domain_with_invalid_field_by_repo(
        self, project_repo
    ):
        # Arrange: Create Project Domain
        proj = ProjectDomainFactory()
    ...
    async def test_project_model_update_domain_missing_required_field_by_repo(
        ...
        with pytest.raises(AttributeError):
            await project_repo.update(default_project)
    ...
    async def test_project_model_remove_non_existed_project_by_repo(
        ...
        with pytest.raises(ValueError):
            await project_repo.delete_by_id(generate_id())
    ...
    async def test_project_model_get_non_existed_project_by_repo(
        ...
        with pytest.raises(ValueError):
            await project_repo.get_by_id(generate_id())
