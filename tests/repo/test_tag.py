...
class TestTagModelCase:
    ...
    async def test_tag_model_create_by_repo(self, repo):
    ...
    async def test_tag_model_update_by_repo(self, repo, default_tag):
        ...
        # Act: Update by repo
        updated_tag = await repo.update(default_tag)

        # Assert: Check if update successfully or not
        assert all(getattr(updated_tag, k) == v for k, v in updated_attrs.items())

    async def test_tag_model_get_by_repo(self, repo, default_tag):
        # Act: Get by repo
        get_tag = await repo.get_by_id(default_tag.id)

        # Assert: Check if get tag as expected
        assert get_tag == default_tag

    async def test_tag_model_list_by_owner_id_by_repo(self, repo, default_tag):
        ...
        for _ in range(NEW_TAGS):
            new_tags.append(await create_tag(repo, owner_id=owner_id))

        # Act: Get tags owned by the same owner
        tags_from_repo = await repo.list_by_owner_id(owner_id)

        assert all(tag in tags_from_repo for tag in new_tags)

    async def test_tag_model_remove_by_repo(self, repo, default_tag):
        # Arrange: Get by repo
        get_tag = await repo.get_by_id(default_tag.id)
        ...
        await repo.delete_by_id(default_tag.id)

        # Assert: Delete tag should not be got, raise ValueError
        with pytest.raises(ValueError):
            await repo.get_by_id(default_tag.id)

    # === SAD tests following ===

    async def test_tag_model_create_domain_with_invalid_field_by_repo(self, repo):
        # Arrange: Create Tag Domain
        tag = TagDomainFactory()
    ...
    async def test_tag_model_create_domain_missing_required_field_by_repo(self, repo):
    ...
    async def test_tag_model_update_with_invalid_field_by_repo(self, repo, default_tag):
        # Arrange: Define attrs
    ...
    async def test_tag_model_update_domain_missing_required_field_by_repo(
        ...
        with pytest.raises(AttributeError):
            await repo.update(default_tag)

    async def test_tag_model_remove_non_existed_tag_by_repo(self, repo, default_tag):
        # Assert: Delete non-existed tag should raise ValueError
        with pytest.raises(ValueError):
            await repo.delete_by_id(generate_id())

    async def test_tag_model_get_non_existed_tag_by_repo(self, repo, default_tag):
